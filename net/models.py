...
class DynamicCNNModel(nn.Module):
    def __init__(self):
        ...
        self.downsample1 = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=3, stride=2, padding=1)
        
        # Block 2
        self.conv2 = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=3, stride=1, padding=1)
        self.downsample2 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, stride=2, padding=1)
        
        # Block 3
        self.conv3 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.downsample3 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1)
        
        # The dense layers will be defined in the forward method since the input size is dynamic
        
        # Output layer
        # Note: The output layer cannot be defined here since the input size is dynamic
    ...
    def forward(self, x):
        ...
        # Dynamically calculate input features for dense layer
        num_features_before_fc = x.size(1) * x.size(2) * x.size(3)  # C * H * W
        
        # Flatten for dense layers
        x = x.view(x.size(0), -1)  # Flatten the tensor for the dense layer
        
        # Define and apply the first dense layer dynamically
        fc1 = nn.Linear(num_features_before_fc, 128).to(x.device)
        x = F.relu(fc1(x))
        
        # Define and apply the second dense layer
        fc2 = nn.Linear(128, 8).to(x.device)
        x = F.relu(fc2(x))
        
        # Define and apply the output layer dynamically
        output = nn.Linear(8, 3).to(x.device)
        x = output(x)
