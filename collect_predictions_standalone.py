...
import os
from datasets import train_classification_cam, val_classification_data
...
def main():
    ...
    model_dir = f'model_weights/classification_efficientnet_cv_{args.mask}/'
    
    print(f'Loading model from {model_dir}')
    print(f'Using device: {device}')
    # Load test dataset (adjust paths as needed)
    test_data = pd.read_csv(args.test_csv)
    test_dataset = val_classification_data(valdir, test_data, crop_size=(256, 256))
    ...
    # Save results
    output_csv = os.path.join('predictions',f'predictions_balanced_train_{args.mask}.csv')
