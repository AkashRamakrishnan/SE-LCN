...
parser = argparse.ArgumentParser(description='Train the lesion classification.')
parser.add_argument('--model_name', type=str, required=True, help='Name of the model to save checkpoints and plots.')
parser.add_argument('--mask', type=str, default='None', help='Type of input masking')
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
    ...
    train_subset = Subset(train_classification_data(datadir, train_data, crop_size=(h, w)), train_idx)
    val_subset = Subset(val_classification_data(datadir, train_data, crop_size=(h, w)), val_idx)
    ...
    testloader = DataLoader(val_classification_data(valdir, test_data, crop_size=(h, w)), batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
